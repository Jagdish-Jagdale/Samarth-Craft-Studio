...
export default function CheckoutPage() {
  ...
  const [selectedAddressId, setSelectedAddressId] = useState(() => {
    if (
      ...
      Array.isArray(userProfile.addresses) &&
  ...
  const calculateShippingCharge = () => {
    if (selectedAddressId === "new") {
      ...
      if (
        currentCity.includes("kolhapur") ||
        currentDistrict.includes("kolhapur")
      ) {
      ...
      if (selectedAddress) {
        ...
        if (
          addressCity.includes("kolhapur") ||
          addressDistrict.includes("kolhapur")
        ) {
          return paymentConfig.shippingRates.local;
  ...
  const handlePincodeChange = async (val) => {
    ...
    if (cleanVal.length === 6) {
      ...
      setPincodeLookupMessage(
        "⚡ Auto-filling address from PIN code directory...",
      );
      try {
        const res = await fetch(
          `https://api.postalpincode.in/pincode/${cleanVal}`,
        );
        ...
        if (
          ...
          setPincodeLookupMessage(
            "⚠ No matching details found for this PIN code.",
          );
        }
      } catch (err) {
        console.error("Failed to fetch pincode details: ", err);
        setPincodeLookupMessage(
          "⚠ Pincode lookup offline. Please enter details manually.",
        );
      } finally {
  ...
  const handleRazorpayPayment = async () => {
    ...
    if (!scriptLoaded) {
      console.log(
        "Razorpay payment gateway is currently unreachable. Falling back to secure simulated gateway.",
      );
    ...
    if (
      ...
      const matched = userProfile.addresses.find(
        (a) => a.id === selectedAddressId,
      );
      if (matched) {
    ...
    try {
      ...
      const options = {
        ...
        prefill: {
          ...
          email:
            userProfile?.email ||
            `${customerName.toLowerCase().replace(/\s+/g, "")}@samartha-customer.com`,
        },
        notes: {
      ...
      console.error(
        "Razorpay Order Creation Failed, falling back to simulated gateway:",
        error,
      );
      setShowSimulatedGateway(true);
      setIsSubmitting(false);
  ...
  const handleOrderSubmission = (e) => {
    ...
    if (selectedAddressId === "new") {
      if (!area || !city || !taluka || !district || !stateField || !pincode) {
        alert(
          "Please complete all fields of your shipping destination address.",
        );
        return;
      }
      if (pincode.length !== 6) {
  ...
  const finalizeOrder = async (razorpayPaymentId = null) => {
    ...
    cart.forEach((item) => {
      if (item.resellerId) {
        // Use base price if available (for commission-added items), otherwise use regular priceou
        const itemPrice =
          item.basePrice ||
        ...
        const commissionAmount = Math.round(
          (itemTotal * parseFloat(commissionRate)) / 100,
        );
    ...
    const newOrder = {
      ...
      companyAddress:
        "263, Shivaji Jadhav Building, Rohidas Chowk, Near Old Bus Stop, Pattan Kadoli, Kolhapur, Maharashtra, Pin 416202",
    };
    ...
    if (firebaseUser?.uid && newAddressObj) {
      try {
        const updatedProfile = await saveUserAddress(
          firebaseUser.uid,
          newAddressObj,
        );
        if (updatedProfile) {
  ...
  const downloadInvoice = (order) => {
    ...
    if (!invoiceWindow) {
      alert(
        "Pop-up blocked! Please allow popups to download/print your invoice.",
      );
      return;
    }
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const newReview = {
      ...
      customerName:
        createdOrder?.customerName || customerName || "Valued Customer",
            ...
            >
              ...
              <div className="space-y-8 bg-white border border-dark/5 p-8 shadow-[0_20px_50px_rgba(26,18,8,0.02)]">
                <div>
                  <h2 className="font-serif text-3xl font-bold mb-1">
                    Customer Checkout Registry
                  </h2>
                  <p className="text-xs text-dark/45 uppercase tracking-widest font-semibold">
                    Bespoke Artisan Shipping Dispatch
                  </p>
                </div>
                ...
                <form onSubmit={handleOrderSubmission} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[10px] text-dark/65 font-bold tracking-wider uppercase mb-2">
                        Customer Full Name
                      </label>
                      <input
                        type="text"
                  ...
                  {userProfile?.addresses &&
                    userProfile.addresses.length > 0 && (
                      <div className="space-y-4">
                        <label className="block text-[10px] text-dark/65 font-bold tracking-wider uppercase">
                          Select Saved Destination Address
                        </label>
                  ...
                  {(selectedAddressId === "new" ||
                    ...
                    <div className="bg-[#faf8f5]/40 border border-dark/5 p-6 rounded-xl space-y-4">
                      <div className="border-b border-dark/5 pb-2 mb-2">
                        <p className="text-xs font-bold text-dark uppercase tracking-wide">
                          ...
                          {pincodeLookupMessage && (
                            <p
                              className={`text-[9px] font-bold mt-1.5 ${
                                pincodeLookupMessage.startsWith("✓")
                                  ? "text-emerald-700"
                                  : pincodeLookupMessage.startsWith("⚡")
                                    ? "text-gold-600 animate-pulse"
                                    : "text-stone-500"
                              }`}
                            >
                              {pincodeLookupMessage}
                    ...
                    <div className="text-xs text-blue-700 space-y-1">
                      <p>
                        ...
                        {(() => {
                          const currentState =
                            selectedAddressId === "new"
                              ? stateField.toLowerCase()
                              : userProfile?.addresses
                                  ?.find((a) => a.id === selectedAddressId)
                                  ?.state?.toLowerCase() || "";
                      ...
                      <p>
                        <strong>Shipping Charge:</strong> ₹{shippingCharge}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-[10px] text-dark/65 font-bold tracking-wider uppercase mb-3">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        {
                          label: "Pay Online",
                          desc: "Secure simulated payment gateway",
                          icon: "💳",
                        },
                        {
                          label: "Cash on Delivery (COD)",
                          desc: `Pay at your doorstep (+₹${paymentConfig.codExtraCharge})`,
                          icon: "🚚",
                        },
                      ].map((pay) => (
                        <div
                        ...
                        >
                          ...
                          <span className="text-[9px] text-dark/45 font-medium">
                            {pay.desc}
                          </span>
                          {pay.label === "Cash on Delivery (COD)" &&
                            paymentMethod === pay.label && (
              ...
              <div className="space-y-6 sticky top-28 h-fit">
                <div className="bg-white border border-dark/5 p-6 shadow-sm space-y-6">
                  <h3 className="font-serif text-lg font-bold border-b border-dark/5 pb-3">
                    Shopping Summary
                  </h3>

                  {cart.length === 0 ? (
                    <div className="text-center py-6 text-xs text-dark/40 font-medium">
                      Your Shopping bag is empty. Select curations from the
                      storefront to complete checkout.
                    </div>
                  ) : (
                    <>
                      <div className="max-h-60 overflow-y-auto space-y-4 pr-1">
                        {cart.map((item, idx) => (
                          <div
                            key={idx}
                            className="flex gap-4 pb-3 border-b border-dark/5 text-xs"
                          >
                            <img
                              src={item.images?.[0] || item.image || item.img}
                              className="w-12 h-12 object-cover border border-dark/5 bg-stone-50"
                              alt=""
                            />
                            <div className="flex-1 min-w-0">
                              <h4 className="font-serif font-bold text-dark truncate">
                                {item.name}
                              </h4>
                              ...
                              <span className="text-[10px] font-bold text-gold-700 mt-1 block">
                                ₹
                                {parseFloat(
            ...
            >
              ...
              <div className="bg-[#faf8f5] p-6 rounded-2xl border border-dark/5 text-left space-y-4 text-xs">
                <div className="border-b border-dark/5 pb-3 flex justify-between font-bold">
                  <span className="text-[10px] text-dark/40 tracking-wider uppercase">
                    Order Details
                  </span>
                  <span className="text-gold-600 font-serif text-sm font-semibold italic">
                    {createdOrder?.date}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="block text-[9px] text-dark/40 font-bold uppercase mb-0.5">
                  ...
                  <div className="text-right">
                    ...
                    <strong className="text-emerald-700 uppercase tracking-wide">
                      Processing Dispatch
                    </strong>
                    ...
                    <p className="text-gold-700 font-serif text-lg font-bold mt-2">
                ...
                <div className="border-t border-dark/5 pt-3">
                  <span className="block text-[9px] text-dark/40 font-bold uppercase mb-2">
                    Curations Ordered List
                  </span>
                  <div className="space-y-2">
                    {createdOrder?.items.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-center text-[11px] text-dark/75"
                      >
                        <span>
              ...
              <div className="bg-white p-6 rounded-2xl border border-gold-500/20 shadow-xs text-left space-y-4">
                <div className="border-b border-dark/5 pb-2">
                  ...
                  <p className="text-[10px] text-dark/50 uppercase tracking-widest font-semibold mt-0.5">
                    Help others discover premium handmade craftsmanship
                  </p>
                </div>
                ...
                {reviewSubmitted ? (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-emerald-600 font-medium">
                      Your review has been verified and featured on our homepage
                      customer gallery.
                    </p>
                  </motion.div>
                ) : (
            ...
            >
              <div className="text-center">
                ...
                <p className="text-[10px] text-dark/45 uppercase tracking-widest font-bold">
                  128-bit Bank Encryption Protocol
                </p>
              </div>
              ...
              <div className="bg-[#faf8f5] p-3 rounded-lg border border-dark/5 flex justify-between items-center text-xs">
                ...
                <strong className="text-gold-700 font-serif text-base">
              ...
              <div className="text-center">
                <span className="text-[9px] text-dark/35 font-medium">
                  💡 Any card credentials will bypass authentication for
